...
use crate::convert_ast::converter::ast_constants::{
  TYPE_VARIABLE_DECLARATOR, VARIABLE_DECLARATOR_DEFINITE_FLAG, VARIABLE_DECLARATOR_FLAGS_OFFSET,
  VARIABLE_DECLARATOR_ID_OFFSET, VARIABLE_DECLARATOR_INIT_OFFSET,
...
impl<'a> AstConverter<'a> {
  pub fn store_variable_declarator(&mut self, variable_declarator: &VarDeclarator) {
    ...
    // flags
    let mut flags = 0u32;
    if variable_declarator.definite {
      flags |= VARIABLE_DECLARATOR_DEFINITE_FLAG;
    }
    let flags_position = end_position + VARIABLE_DECLARATOR_FLAGS_OFFSET;
