...
describe('rollup.watch', () => {
	...
	it('watches a file in code-splitting mode', () => {
		return copy('test/watch/samples/code-splitting', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
	...
	it('recovers from an error on initial build', () => {
		return copy('test/watch/samples/error', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	it('recovers from an error even when erroring dependency was "renamed" (#38)', () => {
		return copy('test/watch/samples/dependency', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	it('ignores files that are not specified in options.watch.include, if given', () => {
		return copy('test/watch/samples/ignored', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	it('only rebuilds the appropriate configs', () => {
		return copy('test/watch/samples/multiple', 'test/_tmp/input')
			.then(() => wait(100))
			.then(() => {
				watcher = rollup.watch([
					{
	...
	it('respects output.globals', () => {
		return copy('test/watch/samples/globals', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	it('treats filenames literally, not as globs', () => {
		return copy('test/watch/samples/non-glob', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	it('updates the right hashes on dependency changes', () => {
		...
		let staticName;
		let chunkName;
		return copy('test/watch/samples/hashing', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
	...
	it('skips filesystem writes when configured', () => {
		let watchChangeCnt = 0;
		return copy('test/watch/samples/skip-writes', 'test/_tmp/input').then(() => {
			watcher = rollup.watch({
				input: 'test/_tmp/input/main.js',
				output: {
					file: 'test/_tmp/output/bundle.js',
	...
	describe('addWatchFile', () => {
		...
		it('respects changed watched files in the load hook', () => {
			const WATCHED_ID = resolve('test/_tmp/input/watched');
			return copy('test/watch/samples/watch-files', 'test/_tmp/input').then(() => {
				watcher = rollup.watch({
					input: 'test/_tmp/input/main.js',
					output: {
						file: 'test/_tmp/output/bundle.js',
		...
		it('respects changed watched modules that are already part of the graph in the transform hook', () => {
			return copy('test/watch/samples/dependencies', 'test/_tmp/input').then(() => {
				watcher = rollup.watch({
					input: 'test/_tmp/input/main.js',
					output: {
						file: 'test/_tmp/output/bundle.js',
		...
		it('does not rerun the transform hook if a non-watched change triggered the re-run', () => {
			const WATCHED_ID = resolve('test/_tmp/input/watched');
			let transformRuns = 0;
			return copy('test/watch/samples/watch-files', 'test/_tmp/input')
				.then(() => wait(100))
				.then(() => {
