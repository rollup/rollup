	...
	it('caches chunk emission in transform hook', () => {
		...
		return rollup
			...
			.then(({ output }) => {
				assert.strictEqual(
					output[0].code,
					`var input = new URL('chunk-f0e7d366b.js', import.meta.url).href;\n\nexport { input as default };\n`
				);
				assert.strictEqual(output[1].fileName, 'chunk-f0e7d366b.js');
	...
	it('caches asset emission in transform hook', () => {
		...
		return rollup
			...
			.then(({ output }) => {
				assert.strictEqual(
					output[0].code,
					`var input = new URL('assets/test-b94d27b99.ext', import.meta.url).href;\n\nexport { input as default };\n`
				);
				assert.strictEqual(output[1].fileName, 'assets/test-b94d27b99.ext');
		...
		return rollup
			...
			.then(({ output }) => {
				assert.strictEqual(
					...
					`console.log('imported');\n\n` +
						`var input = new URL('assets/test-7d48a5826.ext', import.meta.url).href;\n\n` +
						`export { input as default };\n`
				);
				assert.strictEqual(output[1].fileName, 'assets/test-7d48a5826.ext');
			...
			.then(({ output }) => {
				assert.strictEqual(
					output[0].code,
					`console.log('imported');\n\n` +
						`var input = new URL('assets/test-791d43e21.ext', import.meta.url).href;\n\n` +
						`export { input as default };\n`
				);
				assert.strictEqual(output[1].fileName, 'assets/test-791d43e21.ext');
