	...
	includeMarked(modules: Module[]) {
		if (this.treeshake) {
			let addedNewNodes,
				treeshakingPass = 1;
			do {
				timeStart(`- treeshaking pass ${treeshakingPass}`);
				addedNewNodes = false;
				...
				timeEnd(`- treeshaking pass ${treeshakingPass++}`);
			} while (addedNewNodes);
		} else {
