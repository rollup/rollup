...
import { InputOptions, OutputOptions, Plugin, WarningHandler } from '../interfaces';
...
function checkAmd(options: OutputOptions, warn: WarningHandler) {
	if (options.moduleId) {
		if (options.amd)
...
function checkInputOptions(options: InputOptions, warn: WarningHandler) {
	if (
		options.transform ||
		options.load ||
		options.resolveId ||
...
function checkOutputOptions(options: OutputOptions, warn: WarningHandler) {
...
export default function rollup(options: InputOptions) {
	try {
		if (!options) {
		...
		return bundle.build().then(() => {
			...
			function generate(options: OutputOptions) {
				if (!options) {
					throw new Error('You must supply an options object');
				}
				checkOutputOptions(options, warn);
				checkAmd(options, warn);
