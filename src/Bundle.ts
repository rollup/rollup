...
export default class Bundle {
			...
			.then(entryModule => {
				...
				if (this.treeshake) {
					...
					while (!settled) {
						...
						while (i--) {
							...
							while (
								statement.parent &&
								!/Function/.test(statement.parent.type)
							)
				...
				this.externalModules.forEach(module => {
					const unused = Object.keys(module.declarations)
						...
						.filter(
							name =>
								!module.declarations[name].activated &&
	...
	render(options = {}) {
		return Promise.resolve().then(() => {
			...
			let intro = [options.intro]
				.concat(this.plugins.map(plugin => plugin.intro && plugin.intro()))
				.filter(Boolean)
				.join('\n\n');
			...
			let outro = [options.outro]
				.concat(this.plugins.map(plugin => plugin.outro && plugin.outro()))
				.filter(Boolean)
				.join('\n\n');
			...
			if (!finalise) {
				error({
					code: 'INVALID_OPTION',
					message: `Invalid format: ${options.format} - valid options are ${keys(
						finalisers
					).join(', ')}`
				});
			}
	...
	warn(warning) {
		warning.toString = () => {
			...
			if (warning.loc)
				str += `${relativeId(warning.loc.file)} (${warning.loc.line}:${warning
					.loc.column}) `;
			str += warning.message;
