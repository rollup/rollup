// This file is generated by scripts/generate-buffer-parsers.ts.
...
const bufferParsers: ((node: any, position: number, buffer: AstBuffer) => void)[] = [
	...
	function assignmentExpression(node: AssignmentExpression, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]] as ast.AssignmentExpression['operator'];
	...
	function binaryExpression(node: BinaryExpression, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]] as ast.BinaryExpression['operator'];
		node.left = convertNode(node, scope, buffer[position + 1], buffer);
		node.right = convertNode(node, scope, buffer[position + 2], buffer);
	},
	...
	function importDeclaration(node: ImportDeclaration, position, buffer) {
		...
		node.source = convertNode(node, scope, buffer[position + 1], buffer);
		node.attributes = convertNodeList(node, scope, buffer[position + 2], buffer);
		const phasePosition = buffer[position + 3];
		node.phase =
			phasePosition === 0
				? undefined
				: (FIXED_STRINGS[buffer[position + 3]] as ast.ImportDeclaration['phase']);
	},
	...
	function importExpression(node: ImportExpression, position, buffer) {
		...
		const phasePosition = buffer[position + 2];
		node.phase =
			phasePosition === 0
				? undefined
				: (FIXED_STRINGS[buffer[position + 2]] as ast.ImportExpression['phase']);
	},
	...
	function logicalExpression(node: LogicalExpression, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]] as ast.LogicalExpression['operator'];
		node.left = convertNode(node, scope, buffer[position + 1], buffer);
		node.right = convertNode(node, scope, buffer[position + 2], buffer);
	},
	...
	function methodDefinition(node: MethodDefinition, position, buffer) {
		...
		node.kind = FIXED_STRINGS[buffer[position + 4]] as ast.MethodDefinition['kind'];
	},
	...
	function property(node: Property, position, buffer) {
		...
		node.kind = FIXED_STRINGS[buffer[position + 3]] as ast.Property['kind'];
	...
	function unaryExpression(node: UnaryExpression, position, buffer) {
		const { scope } = node;
		node.operator = FIXED_STRINGS[buffer[position]] as ast.UnaryExpression['operator'];
	...
	function updateExpression(node: UpdateExpression, position, buffer) {
		...
		node.operator = FIXED_STRINGS[buffer[position + 1]] as ast.UpdateExpression['operator'];
	...
	function variableDeclaration(node: VariableDeclaration, position, buffer) {
		const { scope } = node;
		node.kind = FIXED_STRINGS[buffer[position]] as ast.VariableDeclaration['kind'];
