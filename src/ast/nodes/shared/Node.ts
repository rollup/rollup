...
export function onlyIncludeSelf<T extends AstNode>(this: NodeBase<T>) {
	this.included = true;
	if (!this.deoptimized) this.applyDeoptimizations();
}

export function onlyIncludeSelfNoDeoptimize<T extends AstNode>(this: NodeBase<T>) {
	this.included = true;
}

export function doNotDeoptimize<T extends AstNode>(this: NodeBase<T>) {
	this.deoptimized = true;
}
