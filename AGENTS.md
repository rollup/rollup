...
- Generated files: Files with "Do not edit this file directly" comments (e.g. `src/ast/bufferParsers.ts`, `src/ast/childNodeKeys.ts`) are generated from `scripts/ast-types.js` via `npm run build:ast-converters` (`scripts/generate-ast-converters.js`) and files that are imported in that file
